...
import com.yotabites.utils.DeltaUtils.{deltaRead, deltaWrite}
...
class DefaultHdfsAPI extends HDFSTransform {
  override def transform(spark: SparkSession, input: DPFConfig): DataFrame = {
    ...
    val df = if (input.format == "delta") {
      deltaRead(spark, input, input.location)
    } else {
      val opts: Map[String, String] = if(input.options == null) Map() else input.options
      spark.read.options(opts).format(input.format).load(input.location)
    }
  ...
  override def save(df: DataFrame, spark: SparkSession, config: Config): (Long, String) = {
    ...
    if (format == "delta") {
      val writeLocation = config.getString("target.options.location")
